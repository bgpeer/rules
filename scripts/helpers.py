...
def _append_ip_to_geosite(name, ip_typed, out_geosite, out_qx_geosite,
    ...
    if cidr_only:
        out_geoip = os.path.join(os.path.dirname(out_geosite), "geoip")
        geoip_list = os.path.join(out_geoip, f"{name}.list")
        # 合并已有 geoip CIDR（避免覆盖 Loyalsoldier 数据）
        existing_cidr = []
        existing_seen = set()
        for line in read_lines(geoip_list):
            if line.startswith("IP-CIDR6,"):
                ...
                continue
            if v.lower() not in existing_seen:
                existing_seen.add(v.lower())
                existing_cidr.append(v)
        merged = existing_cidr + [v for v in cidr_only if v.lower() not in existing_seen]
        if merged:
            mrs_src = os.path.join(workdir, "dl_ip_mrs", f"{name}.txt")
            os.makedirs(os.path.dirname(mrs_src), exist_ok=True)
            write_lines(mrs_src, merged)
            mrs_tasks.append(
...
def cmd_batch_clash_ip(clash_ip_dir, out_geoip, out_qx_geoip,
    ...
    for cyaml in sorted(glob.glob(os.path.join(clash_ip_dir, "*.yaml"))):
        ...
        new_asn = []
        for v in ci_ipcidr:
            if v.lower() not in exist_cidr:
                new_cidr.append(v)
                exist_cidr.add(v.lower())
        for v in ci_asn:
            if v not in exist_asn:
                new_asn.append(v)
                exist_asn.add(v)
        if not new_cidr and not new_asn:
            print(f"[CLASH-IP] {tag}: all entries already covered, skip")
            continue
...
# 旧单命令兼容
# ═══════════════════════════════════════════════════════════════════════════════
def cmd_parse_clash(args):
    yaml_path, out_dir, tag = args
    ...
    for bucket, vals in buckets.items():
        if not vals:
            continue
        dst = os.path.join(out_dir, f"{tag}.{bucket}.txt")
        write_lines(dst, vals)
def cmd_merge_dedup(args):
    geo_file, clash_file, out_file, bucket_type = args
    merged = merge_dedup_lists(read_lines(geo_file), read_lines(clash_file), bucket_type)
    write_lines(out_file, merged)
def cmd_diff_new_entries(args):
    exist_file, new_file, out_file, typ = args
    exist = set()
    for line in read_lines(exist_file):
        parts = line.split(",")
        if len(parts) >= 2:
            exist.add(norm_value(parts[0].strip().upper(), parts[1].strip()))
    out = []
    for line in read_lines(new_file):
        parts = line.split(",")
        if len(parts) < 2:
            continue
        t, v = parts[0].strip().upper(), parts[1].strip()
        if norm_value(t, v) not in exist:
            out.append(line)
    write_lines(out_file, out)
def cmd_rebuild_json_from_list(args):
    list_file, json_dst = args
    # 判断是 geoip 还是 geosite：只要有 DOMAIN 类就按 geosite 重建
    lines = read_lines(list_file)
    has_domain = any(l.startswith("DOMAIN") for l in lines)
    if has_domain:
        _rebuild_geosite_json_from_list(list_file, json_dst)
    else:
        cidrs = []
        for line in lines:
            if line.startswith("IP-CIDR6,"):
                cidrs.append(line[9:].split(",")[0])
            elif line.startswith("IP-CIDR,"):
                cidrs.append(line[8:].split(",")[0])
        ...
        with open(json_dst, "w") as f:
            ...
            f.write("\n")
COMMANDS = {
    "batch_geosite": lambda a: cmd_batch_geosite(*a),
    "batch_geoip": lambda a: cmd_batch_geoip(*a),
    "batch_clash_ip": lambda a: cmd_batch_clash_ip(*a),
    "batch_domain_link": lambda a: cmd_batch_domain_link(*a),
    "batch_ip_link": lambda a: cmd_batch_ip_link(*a),
    "parse_clash": cmd_parse_clash,
    "merge_dedup": cmd_merge_dedup,
    "diff_new_entries": cmd_diff_new_entries,
    "rebuild_json_from_list": cmd_rebuild_json_from_list,
}
def main():
    if len(sys.argv) < 2:
        print("Usage: helpers.py <command> ...", file=sys.stderr)
        sys.exit(1)
    cmd = sys.argv[1]
    args = sys.argv[2:]
