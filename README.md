...
> mrs 格式由 mihomo `convert-ruleset` 编译,天生不支持 keyword / regexp 类型,跳过为正常行为。
>
> QuantumultX 不支持 regexp 类型,相关条目已在转换时自动跳过。
...
- IP 类条目(IP-CIDR / IP-CIDR6 / IP-ASN)→ 融合进 `geo/geosite/`(加 `no-resolve`)+ 额外编译进 `geoip/<名称>.mrs`
...
> 跳过不是丢失,是该格式/软件本身不支持该规则类型,自动过滤以确保兼容性。
>
...
> mrs 格式仅支持 IP-CIDR 类型,IP-ASN 会被跳过。
>
> json/srs(sing-box)和 QX 同样不支持 IP-ASN,自动过滤。
...
```yaml
https://gh.example.org/gist/cfd6fcf7bc40c166984b87ecf4fbf920/raw/Clashmi-fx.yaml
```
